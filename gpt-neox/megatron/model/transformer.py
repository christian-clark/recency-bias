...
class ParallelSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        elif self.pos_emb == "alibi-constant-slope":
            self.alibi_embed = AliBiConstantSlope(
                neox_args.num_attention_heads,
                neox_args.model_parallel_size,
                mpu.get_model_parallel_rank(),
            )
        elif self.pos_emb == "devarda":
            self.alibi_embed = DeVardaBias(
                neox_args.num_attention_heads,
                neox_args.model_parallel_size,
                mpu.get_model_parallel_rank(),
            )
        ...
        if self.sparse:
            ...
            if self.use_flash_attention:
                ...
                if self.pos_emb in ["alibi", "alibi-constant-slope", "devarda"]:
                    raise ValueError(
