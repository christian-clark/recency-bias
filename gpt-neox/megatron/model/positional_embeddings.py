...
class AliBiConstantSlope(AliBi):
    def __init__(self, num_heads, mp_size=1, mp_rank=1):
    ...
    # TODO pass in slope from neox args instead of hardcoding
    def _get_slopes(self, n):
        """
        Get slopes for Alibi positional embedding
        n : int = number of heads.
...
class DeVardaBias(torch.nn.Module):
    def __init__(self, num_heads, mp_size=1, mp_rank=1):
        super().__init__()
        # megatron splits across heads, so we need to make sure each
        # head receives the correct matrix
        ...
        self.cached_matrix = None
        self.cached_seq_len = None
        # parameter values from de Varda and Marelli (2024)
        self.alpha_param = 0.37
        self.lambda_param = 82.86
    ...
    def forward(self, x):
        ...
        #import sys
        #print("CEC x shape:", x.shape, file=sys.stderr)

        # Initialize the AliBi matrix to match the first provided key length; grow it exponentially
        # afterwards if longer inputs are provided. This is important for inference, where we will
        # encounter progressively longer samples; it should have no effect at training time.
        if self.cached_seq_len is not None and self.cached_seq_len >= seq_len_k:
            recency_bias = self.cached_matrix
        else:
            target_seq_len = (
                seq_len_k if self.cached_seq_len is None else self.cached_seq_len * 4
            )
            ij_diff = torch.tril(
                torch.arange(target_seq_len)[:, None] - \
                torch.arange(target_seq_len)[None, :]
            )
            recency_bias = torch.exp(-self.lambda_param * ij_diff)
            recency_bias = recency_bias.unsqueeze(0).repeat(self.num_heads, 1, 1)
            recency_bias = recency_bias.to(x.device).to(x.dtype)
            self.cached_seq_len = target_seq_len
            self.cached_matrix = recency_bias

        # If the bias matrix is larger than the key length, clip it.
        if self.cached_seq_len > seq_len_k:
            recency_bias = self.cached_matrix[:, :seq_len_k, :seq_len_k]
        ...
        if seq_len_q != seq_len_k:
            ...
            recency_bias = recency_bias[:, seq_len_k - 1, :].view(
                recency_bias.shape[0], 1, recency_bias.shape[2]
            ) # seq_len_k - 1 points to the last token index in the current inference batch.

        #print("CEC recency bias shape:", recency_bias.shape, file=sys.stderr)
        #raise Exception
        return (1-self.alpha_param)*x + self.alpha_param*recency_bias
