...
import torch
from transformers import GPTNeoXAlibiConfig, GPTNeoXAlibiForCausalLM
...
def convert(input_checkpoint_path, loaded_config, output_checkpoint_path):
    ...
    hf_model = GPTNeoXAlibiForCausalLM(
        hf_config
    ...
    # CEC
    print(get_state(loaded_tp_ranks, "attention.alibi_embed.slopes", 2))
    print(get_state(loaded_tp_ranks, "attention.alibi_embed.slopes", 3))
    # /CEC
    ...
    for layer_i in tqdm(range(get_key(loaded_config, "num-layers"))):
        ...
        # CEC
        for sd in loaded_tp_ranks:
            print("state dict keys:", sd["module"].keys())

        # /CEC

        # CEC: no rotary embeddings
#        # Just take one
#        state_dict["attention.rotary_emb.inv_freq"] = get_state(
#            loaded_tp_ranks, "attention.rotary_emb.inv_freq", layer_i + 2
#        )[0]
        state_dict["attention.alibi_slopes"] = get_state(
            loaded_tp_ranks, "attention.alibi_embed.slopes", layer_i + 2
        )[0]
...
if __name__ == "__main__":
    ...
    print("CEC attention slopes:")
    print(hf_model.gpt_neox.layers[0].attention.alibi_slopes)
    print(hf_model.gpt_neox.layers[1].attention.alibi_slopes)
